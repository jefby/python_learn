        ...
        for k,v in attrs.iteritems():
            if isinstance(v,Field):
                ...
                if v.primary_key:
                    ...
                    if v.updateable:
                        logging.warning('NOTE: change primary key to non-update')
                        v.updateable = False
                    if v.nullable:
...
class Model(dict):
    ...
    def count_by(cls,where,*args):
        return db.select_int('select count(`%s`) from `%s`' %(cls.__primary_key__.name,cls.__table__,where),*args)
    ...
    def update(self):
        ...
        args.append(getattr(sel,pk))
        db.update('update `%s` set %s whre %s=?' % (self.__table__,','.join(L),pk),*args)
        return self
